...
export async function POST(request: NextRequest) {
  try {
    ...
    // 2. Extract Hotmart data (supports Hotmart Webhook 2.0 and legacy postback)
    const hotmartData = body.data || body;
    const hotmartPurchase = hotmartData.purchase || {};
    const hotmartBuyer = hotmartData.buyer || {};
    const hotmartProduct = hotmartData.product || {};

    const rawEvent = (body.event || hotmartPurchase.status || "PURCHASE_APPROVED").toUpperCase();
    const orderId = hotmartPurchase.transaction || body.transaction || `hotmart_${Date.now()}`;
    const orderStatus = hotmartPurchase.status || rawEvent;

    let eventName: "Purchase" | "InitiateCheckout" | "Refund" = "Purchase";
    if (rawEvent.includes("OUT_OF_SHOPPING_CART") || rawEvent.includes("ABANDON")) {
      eventName = "InitiateCheckout";
    } else if (rawEvent.includes("REFUND") || rawEvent.includes("CHARGEBACK")) {
      eventName = "Refund";
    } else if (rawEvent.includes("APPROVED") || rawEvent.includes("COMPLETE")) {
      eventName = "Purchase";
    }

    // Extract Tracking (sck / src / tracking)
    const tracking = hotmartPurchase.tracking || body.tracking || {};
    const clickId =
      tracking.source_sck ||
      tracking.source ||
    ...
    // 4. Order Amount
    const orderAmount =
      hotmartPurchase.price?.value ||
      hotmartPurchase.original_offer_price?.value ||
      body.cms_marketplace ||
      body.price ||
      0;

    // 5. Customer Data
    const customerEmail = hotmartBuyer.email || body.email || null;
    const customerPhone = hotmartBuyer.checkout_phone || hotmartBuyer.phone || body.phone || null;
    const customerName = hotmartBuyer.name || body.name || null;

    const emailHash = hashEmail(customerEmail);
    const phoneHash = hashPhone(customerPhone);

    const eventId = `hotmart_${orderId}_${eventName.toLowerCase()}`;

    // 6. Meta CAPI Dispatch
    ...
    if (product && product.meta_pixel_id && product.meta_access_token && eventName !== "Refund") {
      const capiResult = await sendMetaCapiEvent({
        ...
        currency: hotmartPurchase.price?.currency_value || "BRL",
        orderId: orderId,
        productName: product.name || hotmartProduct.name,
      });
    ...
    await execute(
      ...
      [
        ...
        hotmartPurchase.price?.currency_value || "BRL",
        orderStatus,
        emailHash,
        phoneHash,
        customerName,
